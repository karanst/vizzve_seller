...
import 'package:eshopmultivendor/Model/UpdateProductStatusModel.dart';
...
class StateProduct extends State<ProductList> with TickerProviderStateMixin {
                  ...
                  Transform.scale(
                      ...
                      child: Switch(
                        onChanged: (bool value) async {
                          if(model.prVarientList![model.selVarient!].status == "1")
                          {
                            setState(() {
                              model.prVarientList![model.selVarient!].status = "0";
                            });
                            UpdateProductStatusModel? statusModel = await updateProductStatus(model.id, model.prVarientList![model.selVarient!].status);
                            if(statusModel!.error == false){
                              setSnackbar(statusModel.message.toString());
                            }
                            print('Switch Button is ON');
                            print(model.prVarientList![model.selVarient!].status);
                            print(model.id);
                          }
                          else
                          {
                            setState(() {
                              model.prVarientList![model.selVarient!].status = "1";
                        ...
                        value: model.prVarientList![model.selVarient!].status == "1" ? true : false,
  ...
  Future<UpdateProductStatusModel?> updateProductStatus(productId, status) async{
    var request = http.MultipartRequest('POST', updateProductStatusApi);
    ...
    if (response.statusCode == 200) {
      ...
      print(str);
      return UpdateProductStatusModel.fromJson(json.decode(str));
    }
    else {
    return null;
    }
