...
class VerifyOtp extends StatefulWidget {
  final String? mobileNumber, countryCode, title, forgotOtp;

  VerifyOtp(
      {Key? key,
      required String this.mobileNumber,
      this.countryCode,
      this.title, this.forgotOtp})
...
class _MobileOTPState extends State<VerifyOtp> with TickerProviderStateMixin {
  ...
  Future<void> getVerifyUser() async {
    try {
      ...
      print(data);
      Response response =
      await post(verifyUserApi, body: data, headers: headers)
  ...
  Future<void> getOtpVerified() async {
    ...
    if (error == true) {
     ...
     /*   id = i[Id];
        ...
        saveUserDetail(id!, username!, email!, mobile!, city!, area!,
            address.toString(), pincode.toString(), latitude.toString(), longitude.toString(), image.toString(), "", "", "", "", "", "", "", "");
*/
